function instant_validation( obj ) {
    ...
    apex.event.trigger(triggeringItem, 'validation-start');
    ...
    if( !itemValidity.valid ) {
        ...
        apex.event.trigger(triggeringItem, 'validation-failure', {
            "validationResult":{
        ...
        ajaxOptions = {
            ...
            success : function( result ) { 
                          if ( result.validationResult.passed == true) {
                              ...
                              apex.event.trigger(triggeringItem, 'validation-success', result);
                          } 
                          else {
                              apex.debug('[Instant validation]: validation for item "' + triggeringItemId + '" ended with failure');
                              if( pluginAttributes.renderError === 'Y' ) {
                              ...
                              apex.event.trigger(triggeringItem, 'validation-failure', result);
                          }
                          
                      },
            error: function(textStatus, errorThrown) { 
                       apex.debug('[Instant validation]: Error: ' + textStatus + '\n' + errorThrown);
                       apex.event.trigger(triggeringItem, 'validation-error');
                   }
        };
